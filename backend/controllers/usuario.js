...
router.put('/username', checkToken, async (req, res) => {
	...
	await DB.query('UPDATE profile SET username=$1 WHERE id=$2', [username, User.id])
...
router.put('/password', checkToken, async (req, res) => {
	...
	let currentPassword = await DB.query('SELECT userpassword FROM profile WHERE id=$1', [User.id])
	currentPassword = currentPassword.rows[0].userpassword
	const currentPasswordValid = await bcrypt.compare(oldpassword, currentPassword)
	...
	if (!(currentPasswordValid && validatePassword.test(password1) && password1 === password2)) {
		return res.status(400).json({ message: 'invalid data' })
	}

	// if all before is correct update the password
	const password_hash = await bcrypt.hash(password1, 10)
	await DB.query('UPDATE profile SET userpassword=$1 WHERE id=$2', [password_hash, User.id])
